...
+(void)displayGateWithCurrentViewController:(UIViewController*)inViewController successBlock:(MKParentalGateSuccessBlock)inSuccessBlock failureBlock:(MKParentalGateFailureBlock)inFailureBlock
{
    ...
    MKParentalGateViewController* gateVC = [[MKParentalGateViewController alloc] initWithTopTargetIcon:[UIImage imageNamed:@"top-dot.png"] bottomTargetIcon:[UIImage imageNamed:@"bottom-dot.png"] successBlock:inSuccessBlock failureBlock:inFailureBlock orientation:UIInterfaceOrientationPortrait];
    
    [inViewController presentViewController:gateVC animated:YES completion:^{
        [(AppDelegate *)[[UIApplication sharedApplication] delegate] setRan:[NSNumber numberWithInt:1]];
    }];
}
